...
class SDU:
    def __init__(self,studentid,password):
        self.loginUrl = 'http://yjs.zjut.edu.cn/NewLoginCheck.asp'
        self.gradeUrl = 'http://yjs.zjut.edu.cn/index.asp?serviceType=lookForGrade-disp'
        ...
        self.head = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36',
                     'Referer': 'http://yjs.zjut.edu.cn/index.asp?serviceType=degreeQualification-Edit',
    ...
    def getGrade(self):
        ...
        for i in range(0,length):
            if self.five[i+1]==u'优' or self.five[i+1]==u'优秀':
                self.five_1.append(4.5)
            elif self.five[i+1]==u'良' or self.five[i+1]==u'良好':
                self.five_1.append(3.5)
            elif self.five[i+1]==u'中' or self.five[i+1]==u'中等':
                self.five_1.append(2.5)
            elif self.five[i+1]==u'及' or self.five[i+1]==u'及格':
                self.five_1.append(1.5)
            else:
                a=int(self.five[i+1])#2017-2-19 12:15:52
                if a<60:
